...
def crop_and_save_image(target_path, rotated_path, output_folder):
    ...
    rotated_keypoints, rotated_descriptors = detector.detectAndCompute(rotated_image, None)

    # Check if descriptors are computed successfully
    if target_descriptors is None or rotated_descriptors is None:
    ...
    matches = matcher.knnMatch(target_descriptors, rotated_descriptors, k=k)
    ...
    rotated_points = np.float32([rotated_keypoints[m.trainIdx].pt for m in good_matches]).reshape(-1, 1, 2)

    # Find the perspective transformation between the matched keypoints
    M, _ = cv2.findHomography(target_points, rotated_points, cv2.RANSAC, 5.0)

    # Apply the perspective transformation to the corners of the rotated image
    ...
    if x < 0 or y < 0 or w < 0 or h < 0:
        border_size = 100
        rotated_image = cv2.copyMakeBorder(rotated_image, border_size, border_size, border_size, border_size,
                                           cv2.BORDER_CONSTANT, value=(255, 255, 255))

        # Update the values of x, y, w, and h based on the added border
        x += border_size
        y += border_size
        w += border_size
        h += border_size
    ...
    # Crop the rotated image based on the bounding box
    cropped_image = rotated_image[y:y+h, x:x+w]

    # Load the target image (smaller cropped JPEG) in color
    target_image = cv2.imread(target_path, cv2.IMREAD_COLOR)

    # Display the cropped image as a preview
    # accept_crop = display_preview(target_image, cropped_image)
    accept_crop = True

    # If the crop was accepted, save the cropped image with the same color profile as the source image
    if accept_crop:
        output_path = os.path.join(output_folder, os.path.basename(rotated_path))
        cv2.imwrite(output_path, cropped_image)
        convert_tif_to_srgb(output_path, output_path)
        print("Cropped image saved:", output_path)
    else:
        print("Crop discarded.")
...
output_folder = "/Users/ericshows/Downloads/maps_cropped_old_method"  # Output folder for cropped images
