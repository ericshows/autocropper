...
def crop_and_save_image(target_path, rotated_path, output_folder):
    ...
    # Calculate the center point of the green rectangle
    center_x = (x_min + x_max) / 2
    center_y = (y_min + y_max) / 2

    # Calculate the rotation center point
    rotation_center = np.float32([[center_x, center_y]])

    # Calculate the rotation matrix based on the rotation angle
    rotation_matrix = cv2.getRotationMatrix2D((center_x, center_y), rotation_deg, 1)

    # Apply the rotation to the transformed corners and rotation center
    rotated_corners = cv2.transform(transformed_corners.reshape(1, -1, 2), rotation_matrix).reshape(-1, 2)
    rotated_center = cv2.transform(rotation_center, rotation_matrix).reshape(-1, 2)

    # Update the green rectangle and circles with the rotated coordinates
    for i, corner in enumerate(rotated_corners):
        ...
        # Update the transformed corners with the rotated coordinates
        transformed_corners[i] = [x, y]

    # Update the rotation center with the rotated coordinates
    center_x, center_y = rotated_center[0]
    rotation_center[0] = [center_x, center_y]
