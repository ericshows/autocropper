...
def crop_and_save_image(target_path, rotated_path, output_folder):
    ...
    # Draw a green rectangle on the rotated image
    cv2.rectangle(rotated_image, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (0, 255, 0), 4)

    # Save the modified image with the green rectangle
    output_path = os.path.join(output_folder, os.path.basename(rotated_path))
    cv2.imwrite(output_path, rotated_image)
    print("Image with green box saved:", output_path)
...
output_folder = "/Users/ericshows/Downloads/maps_boxed"  # Output folder for cropped images
