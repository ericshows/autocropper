...
def crop_and_save_image(target_path, rotated_path, output_folder):
    ...
    # Copy the original image onto the expanded canvas
    expanded_image[int(y_offset):int(y_offset) + rotated_image.shape[0], int(x_offset):int(x_offset) + rotated_image.shape[1]] = rotated_image

    # Adjust the transformed corners by adding the offset values
    adjusted_corners = transformed_corners + np.array([[x_offset, y_offset]])

    # Find the new minimum and maximum coordinates after adjustment
    x_coords = adjusted_corners[:, 0]
    y_coords = adjusted_corners[:, 1]
    ...
    # Calculate the width and height of the output image based on the adjusted region of interest
    output_width = x_max - x_min
    output_height = y_max - y_min

    # Rotate the transformed corners based on the estimated rotation angle
    center_x = (adjusted_corners[0, 0] + adjusted_corners[2, 0]) / 2
    center_y = (adjusted_corners[0, 1] + adjusted_corners[2, 1]) / 2
    rotation_matrix = cv2.getRotationMatrix2D((center_x, center_y), rotation_deg, 1.0)
    rotated_image = cv2.warpAffine(expanded_image, rotation_matrix, (expanded_width, expanded_height))

    # Crop the rotated image to match the dimensions implied by the adjusted region of interest
...
# Folder paths
input_folder = "/Users/ericshows/Downloads/maps/hold"  # Folder containing target and source images
converted_folder = "/Users/ericshows/Downloads/maps_converted"
rotated_folder = "/Users/ericshows/Downloads/maps_rotated"
output_folder = "/Users/ericshows/Downloads/maps_cropped"  # Output folder for cropped images
